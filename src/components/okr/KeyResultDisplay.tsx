...
import { CheckCircle2, ListChecks, MinusCircle, Radio, Edit } from 'lucide-react';
...
export function KeyResultDisplay({ keyResult, lifeOkrId, areaOkrId, onEditKeyResult }: KeyResultDisplayProps) {
  ...
  const getSubTaskIcon = () => {
    ...
    if (keyResult.subTasks.completed > 0) return <Radio className="h-4 w-4 text-primary" />;
    ...
    <div className="flex items-center py-3 px-2 border-b border-border/60 hover:bg-muted/30 transition-colors duration-150">
      ...
      <div className="flex items-center space-x-1.5 flex-shrink-0">
        ...
        <Button
          variant="ghost"
          size="icon"
          onClick={(e) => { e.stopPropagation(); onEditKeyResult(keyResult, areaOkrId, lifeOkrId); }}
